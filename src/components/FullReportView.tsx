...
export const FullReportView: React.FC = () => {
  ...
  const [mobileOutlineOpen, setMobileOutlineOpen] = useState<boolean>(false);
  ...
  const navItems = [
    ...
    { id: 'sec-definitely', label: '2. Definitely Unused (25)' },
    { id: 'sec-probably', label: '3. Probably Unused (11)' },
    { id: 'sec-verification', label: '4. Manual Verification (7)' },
    { id: 'sec-duplicates', label: '5. Duplicate & Redundant (8)' },
    { id: 'sec-deps', label: '6. Unused Dependencies' },
    { id: 'sec-dead-code', label: '7. Dead / Legacy Code (9)' },
    { id: 'sec-recommendations', label: '8. Cleanup Recommendations' },
    { id: 'sec-map', label: '9. Reference Graph Map' },
    { id: 'sec-summary-table', label: 'Final Master Table' },
    { id: 'sec-first-action', label: 'Suggested First Action' },
  ];

  return (
    <div className="flex flex-col lg:flex-row gap-6 animate-in fade-in duration-300" id="full-report-container">
      
      {/* Mobile Collapsible Outline Dropdown (<lg) */}
      <div className="lg:hidden bg-white rounded-2xl border border-black/[0.06] p-3 shadow-[0_1px_3px_rgba(0,0,0,0.03)]">
        <button
          onClick={() => setMobileOutlineOpen(!mobileOutlineOpen)}
          className="w-full flex items-center justify-between text-xs font-semibold text-zinc-900 py-1 px-2 cursor-pointer"
        >
          <div className="flex items-center gap-2">
            <List className="w-4 h-4 text-zinc-600" />
            <span>Document Outline (Jump to Section)</span>
          </div>
          <ChevronDown className={`w-4 h-4 text-zinc-400 transition-transform ${mobileOutlineOpen ? 'rotate-180' : ''}`} />
        </button>
            ...
            {navItems.map((item) => (
              <button
                ...
                className={`w-full text-left px-2.5 py-1.5 rounded-lg text-xs transition-colors cursor-pointer ${
                  activeSection === item.id ? 'bg-zinc-100 text-zinc-900 font-semibold' : 'text-zinc-600 hover:text-zinc-900 hover:bg-zinc-50'
                }`}
              >
                {item.label}
              </button>
      ...
      <main className="flex-1 space-y-5 sm:space-y-6 min-w-0">
        ...
        <section className="bg-white p-4 sm:p-6 rounded-2xl border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-3 sm:space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 text-xs bg-zinc-50 p-3 sm:p-4 rounded-xl border border-zinc-100">
            <div>
            ...
            <div>
              ...
              <a href={PROJECT_METADATA.repo} target="_blank" rel="noreferrer" className="text-[#0071E3] hover:underline font-mono break-all">
        ...
        <section id="sec-overview" className="bg-white p-4 sm:p-6 rounded-2xl border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-4">
          <h2 className="text-sm sm:text-base font-semibold text-zinc-900 flex items-center gap-2 pb-2 border-b border-zinc-100">
          ...
          <div className="space-y-3.5 text-xs leading-relaxed text-zinc-600">
            ...
            <div>
              ...
              <p className="font-mono bg-zinc-50 p-2.5 rounded-xl border border-zinc-100 text-zinc-800 break-words">
        ...
        <section id="sec-definitely" className="bg-white p-4 sm:p-6 rounded-2xl border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-4">
              ...
              <tbody className="divide-y divide-zinc-100">
                {MASTER_AUDIT_ITEMS.filter(i => i.status === 'Definitely unused').map((item, idx) => (
                  <tr key={item.id} className="hover:bg-zinc-50/80">
                    ...
                    <td className="p-2.5 text-zinc-900 font-medium break-all">{item.path}</td>
                    <td className="p-2.5">
        ...
        <section id="sec-probably" className="bg-white p-4 sm:p-6 rounded-2xl border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-zinc-100">
            <h2 className="text-sm sm:text-base font-semibold text-zinc-900 flex items-center gap-2">
        ...
        <section id="sec-verification" className="bg-white p-4 sm:p-6 rounded-2xl border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-zinc-100">
            <h2 className="text-sm sm:text-base font-semibold text-zinc-900 flex items-center gap-2">
          ...
          <div className="space-y-3 font-sans text-xs">
            {VERIFICATION_ITEMS.map((item) => (
              <div key={item.id} className="p-3.5 sm:p-4 rounded-xl bg-zinc-50 border border-zinc-100 space-y-2">
                ...
                <div className="p-2.5 rounded-lg bg-white border border-zinc-200 text-zinc-700 leading-relaxed font-mono text-[11px] break-words">
        ...
        <section id="sec-duplicates" className="bg-white p-4 sm:p-6 rounded-2xl border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-zinc-100">
            <h2 className="text-sm sm:text-base font-semibold text-zinc-900 flex items-center gap-2">
          ...
          <div className="space-y-3 font-sans text-xs">
            {DUPLICATE_GROUPS.map((grp) => (
              <div key={grp.id} className="p-3.5 sm:p-4 rounded-xl bg-zinc-50 border border-zinc-100 space-y-2">
                <div className="flex items-center gap-2">
                  <span className="flex items-center justify-center w-5 h-5 rounded-full bg-zinc-200 text-zinc-800 font-mono font-semibold text-xs shrink-0">
                    {grp.id}
                  </span>
                ...
                <div className="bg-white p-2.5 rounded-lg border border-zinc-200 font-mono text-[11px] text-zinc-700 space-y-0.5 break-all">
                  {grp.files.map((f, i) => (
                    <div key={i}>• {f}</div>
                  ))}
                </div>
                <div className="text-zinc-700 text-[11px] font-mono break-words">
        ...
        <section id="sec-deps" className="bg-white p-4 sm:p-6 rounded-2xl border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-zinc-100">
            <h2 className="text-sm sm:text-base font-semibold text-zinc-900 flex items-center gap-2">
            ...
            <table className="w-full text-left text-xs divide-y divide-zinc-100 font-mono">
              <thead>
                <tr className="bg-zinc-50/70 text-zinc-500 font-sans">
                  ...
                  <th className="p-2.5 w-24 sm:w-28">Verdict</th>
              ...
              <tbody className="divide-y divide-zinc-100">
                {DEPENDENCY_AUDIT_LIST.filter(d => d.status !== 'Confirmed Used').map((dep, idx) => (
                  <tr key={idx} className="hover:bg-zinc-50/80">
                    <td className="p-2.5 font-semibold text-zinc-900 break-all">{dep.name}</td>
                    <td className="p-2.5">
          ...
          <div className="bg-zinc-50 p-3.5 sm:p-4 rounded-xl border border-zinc-100 space-y-2 text-xs font-sans">
            <div className="text-zinc-900 font-semibold font-mono">Confirmed used (keep):</div>
            <p className="text-zinc-600 font-mono text-[11px] leading-relaxed break-words">
        ...
        <section id="sec-dead-code" className="bg-white p-4 sm:p-6 rounded-2xl border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-zinc-100">
            <h2 className="text-sm sm:text-base font-semibold text-zinc-900 flex items-center gap-2">
          ...
          <div className="space-y-2.5 font-sans text-xs">
            {DEAD_CODE_ITEMS.map((item) => (
              <div key={item.id} className="p-3 sm:p-3.5 rounded-xl bg-zinc-50 border border-zinc-100 space-y-1">
                ...
                <div className="font-mono text-zinc-600 text-[11px] break-all">{item.location}</div>
        ...
        <section id="sec-recommendations" className="bg-white p-4 sm:p-6 rounded-2xl border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-4">
          <h2 className="text-sm sm:text-base font-semibold text-zinc-900 flex items-center gap-2 pb-2 border-b border-zinc-100">
          ...
          <div className="space-y-3 sm:space-y-4 text-xs font-sans">
            <div className="p-3.5 sm:p-4 rounded-xl bg-emerald-50/50 border border-emerald-200/70 space-y-2">
              ...
              <ul className="list-disc list-inside space-y-1 text-zinc-700 leading-relaxed font-mono text-[11px] break-words">
            ...
            <div className="p-3.5 sm:p-4 rounded-xl bg-amber-50/50 border border-amber-200/70 space-y-2">
        ...
        <section id="sec-map" className="bg-white p-4 sm:p-6 rounded-2xl border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-4">
          <h2 className="text-sm sm:text-base font-semibold text-zinc-900 flex items-center gap-2 pb-2 border-b border-zinc-100">
            <span className="text-zinc-400 font-mono">9.</span> Dependency and File Reference Map
          </h2>

          <div className="bg-[#1E1E20] p-3.5 sm:p-4 rounded-xl border border-zinc-800 font-mono text-xs text-zinc-300 overflow-x-auto">
            <pre className="text-zinc-200 leading-relaxed whitespace-pre text-[11px] sm:text-xs">
...
scripts/ ── only clean-preload.cjs (dist:*) and generate-icons.js (npm script) are wired; rest are one-off DB tools/docs`}
        ...
        <section id="sec-summary-table" className="bg-white p-4 sm:p-6 rounded-2xl border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-4">
          <h2 className="text-sm sm:text-base font-semibold text-zinc-900 flex items-center gap-2 pb-2 border-b border-zinc-100">
            Final Summary Table
          </h2>
              ...
              <tbody className="divide-y divide-zinc-100">
                {MASTER_AUDIT_ITEMS.map((row) => (
                  <tr key={row.id} className="hover:bg-zinc-50/80">
                    <td className="p-2.5 text-zinc-900 font-medium break-all">{row.path}</td>
                    ...
                    <td className="p-2.5 font-sans text-zinc-600 leading-relaxed">{row.reason}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section id="sec-first-action" className="bg-white p-4 sm:p-6 rounded-2xl border border-blue-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.03)] space-y-3">
